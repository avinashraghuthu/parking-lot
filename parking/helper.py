...
def parse_n_execute_command(ticket_mgr_obj, command_str):
	...
	try:
		if splitted_commd_str[0] == CREATE_PARKING_LOT:
			...
			print ticket_mgr_obj.create_parking_lot(lot_number)
		elif splitted_commd_str[0] == PARK_VEHICLE:
			print ticket_mgr_obj.park_vehicle(splitted_commd_str[1], splitted_commd_str[2])
		elif splitted_commd_str[0] == LEAVE_PARKING:
			lot_number = int(splitted_commd_str[1])
			print ticket_mgr_obj.leave_parking_lot(lot_number)
		elif splitted_commd_str[0] == GET_PARKING_STATUS:
			print ticket_mgr_obj.get_current_parking_status()
		elif splitted_commd_str[0] == GET_REGISTRATION_NO_WITH_COLOR:
			print ticket_mgr_obj.get_vehicle_no_with_color(splitted_commd_str[1])
		elif splitted_commd_str[0] == GET_SLOT_NO_WITH_COLOR:
			print ticket_mgr_obj.get_car_slot_no_with_color(splitted_commd_str[1])
		elif splitted_commd_str[0] == GET_SLOT_NO_WITH_REG_NO:
			print ticket_mgr_obj.get_car_slot_no_with_vehicle_no(splitted_commd_str[1])
	except IndexError, e:
